from numpy import sum as npsum,fliplr,flipud,rot90,array,concatenate;from itertools import chain,product
...
for tile in open("day20.txt").read().split('\n\n'): #For each tile in the input
    ...
    tiles[num_tile] = array([[0if x=='.'else 1for x in line]for line in lines[1:]])
...
for y in range(1,12): #Set up left edge
    ...
    tile = rot90(tile,edges.index(tile_edge)if tile_edge in edges else edges.index(tile_edge[::-1]))
    if get_edges(tile)[0] != tile_edge: tile = fliplr(tile)
...
for y,x in product(range(12),range(1,12)): #Fill Out Rest
    ...
    tile = rot90(tile, 1+(edges.index(tile_edge)if tile_edge in edges else edges.index(tile_edge[::-1])))
...
snek = [(18,0),(0,1),(5,1),(6,1),(11,1),(12,1),(17,1),(18,1),(19,1),(1,2),(4,2),(7,2),(10,2),(13,2),(16,2)]
sea = lambda v:npsum(v)-sum(15for y,x in product(range(94),range(77))if all(v[y+b][x+a]==1for a,b in snek))
